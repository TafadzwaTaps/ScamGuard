...
async def serve_login():
    ...
    return FileResponse(p) if os.path.isfile(p) else JSONResponse({"detail": "login.html not found"}, status_code=404)


@app.get("/register", include_in_schema=False)
async def serve_register():
    p = os.path.join(STATIC_DIR, "register.html")
    return FileResponse(p) if os.path.isfile(p) else JSONResponse({"detail": "register.html not found"}, status_code=404)
